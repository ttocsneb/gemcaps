...
async fn handle_request(conf: &CapsuleConf, mut stream: TcpStream, addr: SocketAddr, logger: &mut Logger) -> GemcapsResult<()> {
    ...
    let request = match request {
        Some(val) => val,
        None => {
            connection.send(Response::bad_request("Invalid request format").to_string()).await?;
    ...
    if applications.len() == 0 {
        connection.send(Response::not_found("Requested application not served here").to_string()).await?;
    ...
    for application in applications {
        ...
        let response = match application {
            ...
            ConfItem::CGI(application) => {
                cgi::process_cgi_request(&request, &conf, application, &addr, &mut logger).await
            },
        ...
        match response {
            ...
            Err(GemcapsError::Io(err)) => {
                let response = match err.kind() {
                    io::ErrorKind::PermissionDenied => {
                        Response::failure_perm("Permission denied")
                    },
                    ...
                    _ => {
                        ...
                        Response::cgi_error("Internal server error")
                    }
                };
                connection.send(response.to_string()).await?;
                logger.access(response.info()).await;
            ...
            Err(err) => {
                connection.send(Response::cgi_error("Internal server error").to_string()).await?;
                logger.error(err.to_string()).await;
                return Ok(());
            },
            Ok(None) => {},
    ...
    connection.send(Response::not_found("Resource not found").to_string()).await?;
    Err(GemcapsError::new("No resource avalable"))
}


pub async fn capsule_main(mut conf: CapsuleConf, logger: &mut Logger) -> GemcapsResult<()> {
